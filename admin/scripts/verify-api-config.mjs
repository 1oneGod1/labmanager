...
const preloadSource = fs.readFileSync(path.join(adminRoot, 'electron', 'preload.js'), 'utf8');
const apiConfigSource = fs.readFileSync(path.join(adminRoot, 'src', 'apiConfig.js'), 'utf8');
const dashboardSource = fs.readFileSync(path.join(adminRoot, 'src', 'AdminDashboard.jsx'), 'utf8');
assert.match(electronSource, /safeJsonRequest/, 'Main process Admin harus memulihkan respons yang disisipi NetSupport.');
assert.match(electronSource, /admin-api-request/, 'REST API Admin desktop harus melalui IPC aman.');
assert.match(preloadSource, /adminApiRequest/, 'Preload harus mengekspos proxy REST terbatas.');
assert.match(apiConfigSource, /electronAPI\?\.adminApiRequest/, 'Frontend harus memakai proxy REST desktop.');
assert.match(dashboardSource, /transports: \['websocket'\]/, 'Realtime Admin harus memakai WebSocket agar tidak disisipi filter web NetSupport.');
assert.doesNotMatch(dashboardSource, /transports: \['websocket', 'polling'\]/);
